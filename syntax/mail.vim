...
" emails
syn region	mailHeader	contains=@NoSpell,@mailHeaderFields start="^From " skip="^\s" end="\v^[-A-Za-z0-9]*([^-A-Za-z0-9:]|$)"me=s-1
syn match	mailHeaderKey	contained contains=@NoSpell,mailEmail "^From\s.*$"
...
syn match	mailHeaderKey	contained "\v(^(\> ?)*)@<=date:" contains=@NoSpell
syn match	mailSubject	contained "\v(^(\> ?)*)@<=subject:.*$" contains=@NoSpell

" Anything in the header between < and > is an email address
syn match	mailHeaderEmail	contained "<.\{-}>" contains=@NoSpell

" Mail Signatures. (Begin with "-- ", end with change in quote level)
syn region	mailSignature	keepend contains=@mailLinks,@mailQuoteExps,@NoSpell start="^\z(\(> \?\)*\)-- $" end="^\z1$" end="^\z1\@!"me=s-1 end="^\z1\(> \?\)\+"me=s-1
